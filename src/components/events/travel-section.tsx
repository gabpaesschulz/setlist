...
import { z } from "zod";
import { Plus, Copy, Check, ExternalLink, Pencil, ArrowRight, X, CheckCircle2, Clock } from "lucide-react";
...
function TravelForm({ eventId, travel, onClose }: { eventId: string; travel?: Travel; onClose: () => void }) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          {errors.departureLocation && <p className="text-xs text-destructive">{errors.departureLocation.message}</p>}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          {errors.arrivalLocation && <p className="text-xs text-destructive">{errors.arrivalLocation.message}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          <Input type="number" step="0.01" min="0" {...register("price")} placeholder="0,00" className="h-9 text-sm" />
        </div>
      </div>
  ...
  if (editing || !travel) {
    return (
      <div className="rounded-2xl border border-border/50 bg-card p-4 shadow-sm">
        <TravelForm eventId={eventId} travel={travel} onClose={() => setEditing(false)} />
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-border/50 bg-card shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-border/40">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground leading-tight">{typeMeta.label}</h3>
            {travel.company && <p className="text-xs text-muted-foreground">{travel.company}</p>}
            {travel.price !== undefined && travel.price > 0 && (
              <p className="text-xs font-semibold text-emerald-600">
                {new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(travel.price)}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1">
            ...
            {travel.outboundDateTime && (
              <p className="text-xs text-muted-foreground mt-0.5">{formatDateTime(travel.outboundDateTime)}</p>
            )}
          </div>
          ...
          <div className="flex-1 text-right">
            ...
            {travel.returnDateTime && (
              <p className="text-xs text-muted-foreground mt-0.5">Volta: {formatDateTime(travel.returnDateTime)}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-1">
        ...
        {travel.locatorCode && (
          <div className="flex items-center justify-between gap-2 py-2.5 border-b border-border/40">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold font-mono text-foreground tracking-widest">{travel.locatorCode}</span>
...
export function TravelEmptyState({ eventId }: { eventId: string }) {
  ...
  if (adding) {
    return (
      <div className="rounded-2xl border border-border/50 bg-card p-4 shadow-sm">
        <TravelForm eventId={eventId} onClose={() => setAdding(false)} />
      </div>
    );
  }
